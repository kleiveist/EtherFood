---
title: EtherFood – Kanon
type: canon
status: working
language: de
updated: 2026-09-10
---

<!-- PYGINDEX:NAVIGATION START -->
[Übergeordnete Übersicht](../index.md)
<!-- PYGINDEX:NAVIGATION END -->

# Kanon

<!-- PYGINDEX:INDEX START -->
## Inhalt

### Bereiche
- [Grundlagen](grundlagen/index.md)
- [Handlung](handlung/index.md)
- [Welt](welt/index.md)
<!-- PYGINDEX:INDEX END -->

Hier liegt die maßgebliche Quelle für Begriffe, Welt und Handlung von
`EtherFood`. Offene Stellen werden ausdrücklich als offen markiert. Bereits
angenommene Festlegungen werden nur mit einer nachvollziehbaren
[Spielentscheidung](../decisions/index.md) geändert.

## Empfohlener Einstieg

1. [Begriffe und Namen](grundlagen/begriffe-und-namen.md)
2. [Kanon und offene Fragen](grundlagen/kanon-und-offene-fragen.md)
