...
| Aktuell dokumentiert | 6 Animationstypen; 5 davon vollständig in 8 Richtungen |
| Belegte Richtungsplätze | 41 von 200 |
| Dokumentierte GIFs | 43 |
...
GIF-Vorschauen der Figur. Stehen, langes Warten, Gehen, Laufen und Rennen liegen
in allen acht Richtungen vor. Genervtes Warten ist als dreiteilige Folge nach
Süden dokumentiert. Rennen verwendet die Bildfolgen der Sprint-Exporte mit
verdoppelter Wiedergabegeschwindigkeit.
...
| Spielstand | Green Hero; Stehen, langes Warten, Gehen, Laufen und Rennen vollständig; genervtes Warten nur nach Süden |
| Technische Merkmale | Alle GIFs mit 640 × 640 Pixeln; 35 GIFs mit 16 Frames und 1,92 Sekunden Laufzeit; Rennen mit 6 Frames je Richtung, Südosten mit 8 Frames |
| Race-Wiedergabe | 6 Hundertstelsekunden je Frame, etwa 16,7 FPS; exakt doppelte Bildrate der Sprint-Quellen |
...
Die acht Race-GIFs wurden aus den vorhandenen Sprint-GIFs erzeugt. Bildinhalt
und Framefolge blieben unverändert; ausschließlich die Frameverzögerung wurde
von 12 auf 6 Hundertstelsekunden halbiert. Die passend benannten
Race-Spritesheets verbleiben in der lokalen Arbeitsstruktur.
...
Die Race-GIFs besitzen in sieben Richtungen je 6 Frames und laufen 0,36
Sekunden. Südosten besitzt 8 Frames und läuft 0,48 Sekunden. Die Zeile
„Sprinten“ bleibt leer, weil die Sprint-Quellen nicht als eigene
Dokumentations-GIFs übernommen wurden.
...
| Aktuell durch GIFs belegte Animationstypen | 6 |
| Davon vollständig in acht Richtungen | 5 |
| Aktuell belegte Richtungsplätze | 41 |
| Aktuell vorhandene GIF-Dateien | 43 |
| Aktuell in GIFs dokumentierter Umfang | 610 Frames |
| Derzeit ohne GIF-Vorschau | 159 Richtungsplätze |
...
dieser Seite nur die 43 eingebetteten GIF-Dateien. Die drei Teile des genervten
Wartens zählen als ein Animationstyp und ein belegter Richtungsplatz, aber als
drei einzelne GIF-Dateien mit insgesamt 48 Frames. Die acht Race-GIFs steuern
weitere 50 Frames bei; deshalb weicht der tatsächliche Umfang vom theoretischen
16-Frame-Raster ab.
...
`run/` und `race/`. Die dreiteilige Vorschau des genervten Wartens verwendet
die Pfade `stand-very-long-1/`, `stand-very-long-2/` und
